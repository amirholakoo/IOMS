#!/bin/bash
# 🍓 Raspberry Pi Setup for HomayOMS v200 - FIXED VERSION
# 🔧 Complete setup and deployment script for Raspberry Pi with GPG error fix

set -e

echo "🍓 Raspberry Pi Setup for HomayOMS v200 - FIXED VERSION"
echo "======================================================"
...
# Fix GPG error first
echo "🔧 Fixing GPG repository issues..."
sudo apt-get clean
sudo apt-get update --fix-missing

# Remove problematic repository if it exists
if [ -f /etc/apt/sources.list.d/seeed-studio.list ]; then
    echo "🗑️ Removing problematic Seeed Studio repository..."
    sudo rm /etc/apt/sources.list.d/seeed-studio.list
fi
