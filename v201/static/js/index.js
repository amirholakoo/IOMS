...
async function fetchWorkingHoursConfig() {
    try {
        ...
        console.warn('⚠️ Could not fetch working hours config, using default values:', error);
        // Keep default values
    }
}
...
function updateCashPurchaseStatus() {
    ...
    const cashPurchaseBtn = document.getElementById('cashPurchaseBtn');
    ...
    if (CASH_PURCHASES_DISABLED) {
        ...
        // Disable cash section
        if (cashSection) {
            const buttons = cashSection.querySelectorAll('.selection-btn');
            buttons.forEach(btn => {
                btn.disabled = true;
        ...
        // Disable cash purchase button
        if (cashPurchaseBtn) {
            cashPurchaseBtn.disabled = true;
            cashPurchaseBtn.style.opacity = '0.6';
            cashPurchaseBtn.style.cursor = 'not-allowed';
            cashPurchaseBtn.title = 'در حال حاضر امکان خرید نقدی غیرفعال می‌باشد';
        }
    } else {
        // Hide alert
        if (cashPurchaseDisabledAlert) {
        ...
        // Enable cash purchase button (but keep it disabled if no items selected)
        if (cashPurchaseBtn) {
            cashPurchaseBtn.style.opacity = '';
            cashPurchaseBtn.style.cursor = '';
            cashPurchaseBtn.title = '';
            // Only enable if items are selected
            cashPurchaseBtn.disabled = selectedItems.cash === 0;
        }
    }
}
...
function handleCashPurchase() {
    if (CASH_PURCHASES_DISABLED) {
        showAlert('در حال حاضر امکان خرید نقدی غیرفعال می‌باشد', 'error', 'خرید نقدی');
        return;
    }
