...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 2️⃣ تولید خودکار شماره تلفن یکتا برای سوپر یوزر
        if not phone:
            while True:
                phone = f"0999{random.randint(1000000, 9999999)}"
                if not self.UserModel.objects.filter(phone=phone).exists():
                    break
        user_data['phone'] = phone
        
        # 3️⃣ تولید خودکار ایمیل یکتا برای سوپر یوزر
        if not email:
            while True:
                email = f"superuser_{random.randint(100000, 999999)}@dummy.com"
                if not self.UserModel.objects.filter(email=email).exists():
                    break
