...
class Product(BaseModel):
    ...
    breaks = models.IntegerField(
        ...
        help_text="تعداد پارگی‌های موجود در محصول"
    )
    ...
    def clean(self):
        ...
        # 💔 بررسی تعداد پارگی منفی نباشد
        if self.breaks < 0:
            raise ValidationError({
                'breaks': '💔 تعداد پارگی نمی‌تواند منفی باشد'
            })
...
class WorkingHours(BaseModel):
    ...
    # 💰 غیرفعال کردن خرید نقدی
    cash_purchases_disabled = models.BooleanField(
        default=False,
        verbose_name="💰 غیرفعال کردن خرید نقدی",
        help_text="اگر فعال باشد، مشتریان نمی‌توانند محصولات را به صورت نقدی خریداری کنند"
    )
    ...
    def get_working_hours_info(self):
        ...
        return {
            ...
            'cash_purchases_disabled': self.cash_purchases_disabled,
